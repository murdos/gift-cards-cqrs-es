...
[.notes]
--
- Regardons à nouveau comme notre agrégat est construit : uniquement avec l'historique des évènements passés.
...
[.notes]
--
- Cette projection est interne à l'agrégat, et non visible depuis l'extérieur. C'est un détail d'implémentation.
- *click*
...
- Voilà, je pense que nous avons fait le tour des fonctionnalités que nous avons implémenté, et des commandes, prises de décision, génération des évents, calcul d'une projection, et query.
- Comme je l'évoquais tout à l'heure, un modèle de lecture est réalisé sur mesure par rapport à un besoin. Il serait donc intéressant d'étudier une nouvelle fonctionnalité pour aller plus loin.
--
