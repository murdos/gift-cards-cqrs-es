...
On va utiliser un modèle de lecture, également appelé "read model ou ""query model", dédié à notre use case.
--
...
- Comment est-il calculé ? (event handler)
--
...
[.notes]
--
- A partir de l'event initial, GiftCardCreated, on initialise le modèle de lecture
--
...
[.notes]
--
- Et pour chacun des évènements suivants, on va, si besoin, mettre à jour le modèle de lecture.
- Pour cela on charge l'état courant du modèle, auquel on applique le nouvel événement.
--
