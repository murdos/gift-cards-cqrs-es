[transition="slide-in fade-out"]
== Comment afficher la liste des cartes cadeaux ?

[.same]
- Paginée 🤔

[.notes]
--
- Effectivement comment on fait pour afficher une liste de toutes mes cartes cadeaux avec quelques infos, comme le montant restant et la boutique de la carte ?
- Alors que ce qu'on a stocké ce sont des évènements unitaires, dont les informations clés sont dans un payload.
- Relire tous les évènements pour reconstituer toutes les cartes cadeaux à chaque fois paraît compliqué...
- Et si en plus la liste doit être paginée pour n'afficher que 10 cartes cadeaux à la fois
 *click*
--

[transition="fade"]
=== Comment afficher la liste des cartes cadeaux ?

[.same]
- Paginée 🤔
- Triée 😮

[.notes]
--
ou triée, par exemple le montant restant
*click*
--

[transition="fade-in slide-out"]
=== Comment afficher la liste des cartes cadeaux ?

[.same]
- Paginée 🤔
- Triée 😮
- Filtrée 😱

[.notes]
--
Voir même filtrée, par exemple sur la boutique
--

[transition="fade-in slide-out"]
=== Une projection !

image::projection.png[]

[.notes]
--
- On va utiliser une projection, également appelée "query model", ou "read model ou "modèle de lecture".
- Ce modèle va être fabriqué sur mesure par rapport à notre besoin en lecture, à partir des évènements métier.
- Dans notre cas, il va être très similaire au modèle qu'on aurait eu en state-sourcing.
- Mais la grosse différence, c'est que c'est une information dérivée, qui peut être recalculée en fonction de l'évolution des besoins.
- D'ailleurs dans notre implémentation, les projections sont persistées avec des repositories in memory et recaculées à chaque redémarrage de l'application. C'est très pratique au début de l'application, cela offre une grande souplesse.
--

[%notitle]
[transition="slide-in fade-out"]
=== Illustration : 0st event

image::fold00.png[]

[.notes]
--
Si on déroule notre histoire au fur et à mesure, cela donne ça.
--

[%notitle]
[transition="fade"]
=== Illustration : 1st event

image::fold01.png[]

[.notes]
--
A partir de notre premier évènement, la déclaration d'une carte, on va pouvoir initialiser la projection avec le code-barre, et la boutique. Le montant restant est égal au montant initial. Et la carte n'est pas épuisée.
--

[%notitle]
[transition="fade"]
=== Illustration : 2nd event

image::fold02.png[]

[.notes]
--
Ensuite lors d'un paiement, on va mettre à jour le montant restant de la carte cadeau, en retranchant au montant restant le montant du nouveau paiement.
--

[%notitle]
[transition="fade"]
=== Illustration : 3rd event

image::fold03.png[]

[.notes]
--
Ici, même principe pour le second évènement de paiement.
--

[%notitle]
[transition="fade-in slide-out"]
=== Illustration : 4rd event

image::fold04.png[]

[.notes]
--
Enfin lorsque la carte est épuisée, on va mettre à jour le champ "exhausted" de la projection.
Ce qui permettra de l'afficher de manière distincte et d'empêcher de l'utiliser pour un nouveau paiement.
--

=== Code
🔍

=== Query Model

[source, java, highlight]
----
include::../../../src/main/java/io/craft/giftcard/giftcard/domain/projections/currentstate/GiftCardCurrentState.java[lines='12..18']
----

[.notes]
--
- Côte code, le modèle de lecture s'appelle GiftCardCurrentState
- Il contient les informations utilisées pour afficher dans la liste des cartes cadeaux, ou sur lesquelles on souhaiterait filtrer.
- Comment ce modèle est-il calculé ? Vous vous souvenez de ce que Cédric disait précédemment, que l'event était propagé ? Et bien il est propagé sur jusqu'aux events handlers.
--

=== Event handler

[source, java, highlight='..|7..8']
----
include::../../../src/main/java/io/craft/giftcard/giftcard/domain/projections/currentstate/GiftCardCurrentStateUpdater.java[lines='8..9,17..26']
----

[.notes]
--
- L'event handler va être appelé à chaque fois qu'un événement est émis, et va pouvoir mettre à jour le modèle de lecture
- *click*
- On va distinguer deux types d'events : le premier, et les suivants
--

[%notitle]
=== Event handler: init

[source, java, highlight="3|4|2"]
----
include::../../../src/main/java/io/craft/giftcard/giftcard/domain/projections/currentstate/GiftCardCurrentStateUpdater.java[lines='8..9,28..30']
----

[source, java, highlight="3|4.."]
----
@QueryModel
public record GiftCardCurrentState(...) {

include::../../../src/main/java/io/craft/giftcard/giftcard/domain/projections/currentstate/GiftCardCurrentState.java[lines='19..27']
----

[.notes]
--
- C'est à partir du 1er évènement de l'histoire, `GiftCardDeclared`, qu'on initialise le modèle de lecture
- *click*
- On va créer une nouvelle instance de `GiftCardCurrentState` avec les données de l'évènement
--

[%notitle]
=== Event handler: update

[source, java, highlight="3..|4..6|7|2"]
----
include::../../../src/main/java/io/craft/giftcard/giftcard/domain/projections/currentstate/GiftCardCurrentStateUpdater.java[lines='8..9,32..37']
----

[source, java, highlight="3|5,13|6..8|9"]
----
include::../../../src/main/java/io/craft/giftcard/giftcard/domain/projections/currentstate/GiftCardCurrentState.java[lines='12..13,35..47']
----

[.notes]
--
- Et pour chacun des évènements suivants, on va, si nécessaire, mettre à jour le modèle de lecture.
- *click*
- Pour cela on charge l'état courant du modèle,
- *click*
- auquel on applique le nouvel événement.
- *click*
- on fait du pattern matching pour traiter de manière exhaustive tous les types d'événements
--

=== Enregistrement des event handlers

[source, java, highlight="..|10..11"]
----
include::../../../src/main/java/io/craft/giftcard/giftcard/application/GiftCardApplicationService.java[lines='24..26,32..34,36..37,42..44,46']
----

[.notes]
--
- Les events handlers s'abonnent aux nouveaux évènements via l'EventPublisher, au niveau de l'application service
--
